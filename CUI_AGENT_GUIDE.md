...
样式依赖同样必须自包含且最小化：Capture 仅保留静态 Type/Id/Class 选择器能匹配已复制节点的规则，并按源顺序
携带这些规则实际引用的资源。Paste 只有在目标相关规则序列及资源值完全一致时才可复用；否则必须为每个粘贴
...
实时 XAML 编辑的每个未提交区段必须由严格文档事务包围。每次防抖预览先在隔离模型中完成解析和验证，失败只更新诊断，
不能覆盖最后一次有效预览；有效候选的应用若失败必须恢复本次应用前文档和完整选择。接受时提交当前最终端点；
取消/关闭回滚当前区段，禁止为每次键入创建历史项。“应用”/`Ctrl+S` 是显式区段边界：必须先完成同一验证路径，
再 Commit 当前事务并立即以该端点 Begin 下一事务；有变化时只增加一条 Undo，无变化时不得污染历史。应用后取消
只回滚最近区段，之前检查点必须保留且可逐条撤销；由于最终 Rollback 不发布文档变更，外层关闭路径必须显式刷新
Undo/Redo 可用状态。若从未应用，取消仍恢复打开编辑器前的文档和完整选择。格式化必须复用
`XamlDocumentSerializer` 的规范输出，不能另写近似 writer；写回必须使用 RichTextBox 的原子全文替换，
把格式化前选区与按稳定身份映射后的选区保存在同一个 Undo/Redo 记录中，禁止直接赋值 `Text` 绕过历史。
最后有效源码检查点只能在完整预览成功或规范格式化成功后更新。当前源码偏离检查点时必须提供显式“还原预览”
入口；还原要通过 RichTextBox 的原子全文替换写成单个 Undo 记录，并保存调用前选区与还原后选区，使 Undo/Redo
恢复两侧文本和选区。不得用 `SelectAll()` 加普通插入把全选状态泄漏进撤销记录，也不得用直接 `Text` 赋值绕过历史。
语法与语义错误都必须通过 `XamlDocumentDiagnostic` 传递用户消息、1-based 行/列和 UTF-16 编辑器偏移；
语法错误不得直接把 XmlLite 的 UTF-8 字节列数当作 `RichTextBox` 字符索引，语义错误必须通过原始源码的
DOM 旁路映射优先落在当前属性名、其次落在元素名，不能从本地化错误文本反推位置。防抖校验不得自动移动光标，
只有显式的“定位错误”/`F8` 才可选中目标并调用 `ScrollSelectionIntoView()`；任何新键入必须废弃旧诊断位置。
查找/替换、错误定位和行列反馈必须复用 `TextEditCore` 的 UTF-16 搜索/坐标语义与 `RichTextBox::OnSelectionChanged`；
不得在对话框中另写字节索引或只监听键盘。向前/向后查找均须可循环，大小写开关须同时影响查找和替换；
全部替换必须先计算完整结果，再经一次选区替换写回，使 RichTextBox 只记录一个 Undo 单元。成功查找必须覆盖
陈旧的失败提示，程序化选中后须滚动到视口并更新用户可见的 1-based 行列。
源码多行 `Tab` / `Shift+Tab` 必须以触及的完整行为单位增加/减少缩进，不能让 RichTextBox 的普通 Tab 输入覆盖
整个选区；选区恰好结束在换行符后时不得误改下一行。行范围、混合 Tab/空格反缩进和新选区计算应放在
`XamlEditorAssist::BuildLineIndentEdit()` 纯模型中，对话框只用一次 `InsertTextAndSelect()` 写回，从而让文本、
操作后选区和 Undo/Redo 都属于同一个编辑记录。源码右键命中现有选区内部必须保留选区，命中外部才移动光标；
右键、菜单键与 `Shift+F10` 必须复用同一个 `ContextMenu` 和命令处理器，并在每次打开前根据选区、剪贴板与
Undo/Redo 栈刷新可用状态。格式化按钮、菜单和 `Ctrl+Shift+F` 也必须进入同一规范 writer 路径。
XAML 语法着色必须是独立于文本和编辑历史的 RichTextBox 前景样式层：扫描器按 UTF-16 偏移容错识别处理指令、
元素、属性、值、注释、CDATA、声明和实体，不得因未闭合引号/标签或属性值内的 `>` 停止后续安全扫描；文本
变化、格式化和 Undo/Redo 后都要从当前源码重算。真实选区前景必须最后应用，标签配对继续使用独立背景范围。
虚拟化布局必须把全局样式范围裁剪到每个文本块的局部坐标；DirectWrite drawing effect 不得复用会被后续
`SetColor` 改写的共享画刷，同一布局内的每种前景色须引用稳定的独立画刷，并在设备上下文变化时重建缓存。
XAML 补全不得维护第二份内置控件/属性/事件常量表：候选必须从 `ControlRegistry`、属性目录、事件目录和
已安装自定义控件 descriptor 投影。词法上下文、标签扫描/配对、候选过滤和换行缩进必须放在无窗口纯模型中；
弹出层只负责展示与键盘/鼠标提交。属性补全须一次替换为 `Name=""` 并把光标放入引号，自动换行和补全提交
都只能形成一个 RichTextBox Undo 单元。事件值候选必须复用 `DesignDocumentEventIndex` 和用户代码检查中的
同签名函数，不能只提示 `Auto`，也不能接受已被其他签名占用的名称。辅助高亮不得改写真实选区或影响复制/替换语义。
补全弹窗由对话框持有却以编辑器作为定位父级时，必须在 `OnFormClosed` / 子控件清理之前解除 Form 的
`ForegroundControl`、选择与悬停引用并销毁；不能等派生析构函数在定位父级已经释放后再调用 `Hide()`。
XAML 元素与设计选区的双向定位以 `DesignId` 为第一身份、`x:Name` 为兼容后备；`d:Member name=...` 等元数据
元素不得冒充控件。实时预览在源码重命名后也必须按 stable ID 恢复选择，格式化须恢复同一元素而不是跳到顶部。
使用 Right/Bottom Anchor 的对话框控件必须显式设置 `Margin.Right/Bottom` 为锚距，否则兼容布局会把同类按钮重叠到右下角。
