...
CUI provides 46+ common UI controls covering various development needs:
...
- `NumericUpDown` - Numeric step input
...
- `TreeView` - Tree view
- `GridView` - Data grid
- `PagedGridView` - Paged GridView wrapper
- `ListView` - List/details/tile/icon view
- `ListBox` - Simple list selection box
- `PropertyGrid` - Runtime property grid

**Navigation and Date:**
- `NavigationView` / `SideBar` - Sidebar navigation
- `BreadcrumbBar` - Breadcrumb path navigation
...
- `ToastHost` - Page-level notification host
- `MessageDialog` - CUI-styled modal dialog
...
```cpp
auto number = AddControl(new NumericUpDown(20, 520, 120, 28));
number->Min = 0;
number->Max = 100;
number->Step = 5;
number->Value = 25;

number->OnValueChanged += [](NumericUpDown* sender, double oldValue, double newValue) {
    // Keyboard, mouse wheel, and spin buttons all trigger this event
};
```
...
```cpp
auto progress = AddControl(new ProgressBar(20, 560, 200, 24));
...
```cpp
auto expander = AddControl(new Expander(L"Advanced settings", 20, 450, 360, 150));
expander->Padding = Thickness(12);
expander->AddControl(new Label(L"Collapsible content", 0, 0));
expander->AddControl(new NumericUpDown(0, 36, 120, 28));

expander->OnExpandedChanged += [](Expander* sender, bool expanded) {
...
```cpp
auto list = AddControl(new ListView(20, 20, 320, 220));
list->ViewMode = ListViewViewMode::Details;
list->SelectionMode = ListViewSelectionMode::Multiple;
list->ShowCheckBoxes = true;
list->AddColumn(ListViewColumn(L"Name", 160));
list->AddColumn(ListViewColumn(L"State", 100));

ListViewItem item(L"Task 1", L"Ready");
item.Checked = true;
item.SubItems.push_back(item.SubText);
list->AddItem(item);

auto listBox = AddControl(new ListBox(360, 20, 180, 220));
listBox->AddItem(ListViewItem(L"All"));
listBox->AddItem(ListViewItem(L"Today"));
listBox->AddItem(ListViewItem(L"Done"));
```

#### PropertyGrid

```cpp
auto props = AddControl(new PropertyGridView(20, 260, 300, 260));
props->AddProperty(L"Appearance", L"Title", L"Settings", PropertyGridValueType::Text);
props->AddProperty(L"Appearance", L"Enabled", L"True", PropertyGridValueType::Bool);

PropertyGridItem mode(L"Behavior", L"Mode", L"Normal", PropertyGridValueType::Enum);
mode.Options = { L"Compact", L"Normal", L"Roomy" };
props->AddItem(mode);

props->OnValueChanged += [](PropertyGridView* sender, int index,
    std::wstring oldValue, std::wstring newValue) {
    // index maps to sender->Items[index]
};
```

#### NavigationView / SideBar / BreadcrumbBar

```cpp
auto nav = AddControl(new NavigationView(20, 20, 220, 360));
nav->HeaderText = L"Workspace";
nav->AddHeader(L"Main");
nav->AddItem(L"Overview", L"overview");
nav->Items.back().BadgeText = L"3";
nav->AddItem(L"Assets", L"assets");
nav->AddSeparator();
nav->AddItem(L"Settings", L"settings");
nav->SelectItem(1);

nav->OnItemClick += [](NavigationView* sender, int index) {
    // sender->Items[index].Value can be used as a route key
};

auto breadcrumb = AddControl(new BreadcrumbBar(260, 20, 360, 32));
breadcrumb->SetPath({ L"App", L"Assets", L"Details" });
breadcrumb->OnItemClick += [](BreadcrumbBar* sender, int index) {
    // Click any segment to navigate back to that level
};
```

#### CalendarView / DateRangePicker

```cpp
auto calendar = AddControl(new CalendarView(20, 20, 320, 280));
calendar->SelectionMode = CalendarSelectionMode::Range;
calendar->OnSelectionChanged += [](CalendarView* sender) {
    auto range = sender->GetRange();
    if (range.HasStart && range.HasEnd) {
        // range.Start / range.End are the selected inclusive range
    }
};

auto rangePicker = AddControl(new DateRangePicker(L"Select date range", 360, 20, 240, 30));
rangePicker->OnRangeChanged += [](DateRangePicker* sender) {
    auto range = sender->GetRange();
    // sender->Text automatically displays yyyy-mm-dd - yyyy-mm-dd
};
```
...
```cpp
auto color = AddControl(new ColorPicker(20, 330, 220, 30));
color->SelectedColor = D2D1_COLOR_F{ 0.20f, 0.52f, 0.95f, 1.0f };
color->OnColorChanged += [](ColorPicker* sender, D2D1_COLOR_F oldColor,
                            D2D1_COLOR_F newColor, std::wstring value) {
    // value is #RRGGBB or #AARRGGBB
};
```
...
```cpp
auto paged = AddControl(new PagedGridView(20, 450, 640, 360));
paged->PageSize = 50;
paged->AddColumn(GridViewColumn(L"Name", 150, ColumnType::Text));
paged->AddColumn(GridViewColumn(L"Status", 120, ColumnType::ComboBox));

for (int i = 0; i < 500; i++) {
    GridViewRow row;
    row.Cells = { L"Item " + std::to_wstring(i + 1), L"Ready" };
    paged->AddRow(row);
}
paged->RefreshPage();
paged->OnPageChanged += [](PagedGridView* sender, int oldPage, int newPage) {
    // sender->Grid is a full GridView with button/check/combobox columns
};
```
...
```cpp
auto toastHost = AddControl(new ToastHost(460, 40, 340, 260));
toastHost->ShowToast(L"Saved", L"Settings were written to disk.", ToastKind::Success);

toastHost->OnToastClick += [](ToastHost* sender, int index) {
    // index maps to sender->Toasts[index]
};
```
...
auto result = MessageDialog::Show(
    L"Confirm",
    L"Continue with this operation?",
...
| NumericUpDown | Value, Min, Max, Step | OnValueChanged |
| ColorPicker | SelectedColor, ValueText | OnColorChanged |
| GridView | Columns, Rows, Selection | OnSelectionChanged |
| PagedGridView | Rows, PageIndex, PageSize, Grid | OnPageChanged |
| ListView/ListBox | Items, Columns, SelectedIndex | SelectionChanged, OnItemClick |
| PropertyGridView | Items, SelectedIndex, AllowEditing | OnValueChanged, SelectionChanged |
| TreeView | Root, SelectedNode | OnSelectionChanged |
| Expander | IsExpanded, HeaderHeight | OnExpandedChanged |
| NavigationView/SideBar | Items, SelectedIndex, IsPaneOpen | SelectionChanged, OnItemClick |
