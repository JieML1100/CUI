...
实时 XAML 编辑器会将 XML 语法错误，以及无效属性值、未知控件、重复名称等语义错误显示为 1-based 行/列
诊断，并启用“定位错误”。语义诊断优先定位到出错属性名，否则定位到对应元素名；点击按钮或按 `F8` 会选中
目标字符并把长文档滚动到对应行。输入新内容时会立即清除旧定位，有效预览后按钮自动禁用。中文等非 ASCII
内容前的错误使用 UTF-16 编辑器偏移，不会把 UTF-8 字节列数误当作光标位置；所有失败仍保持最后一次有效预览。
源码会实时区分处理指令、元素名、属性名、属性值、注释、CDATA、声明和实体引用；容错扫描在引号未闭合、
字符串包含 `>` 或标签尚未输入完整时仍能稳定着色。颜色是独立的非编辑前景层，不写回文本、不增加 Undo，
真实选区前景和标签配对背景继续覆盖在其上；超过虚拟化阈值的长 XAML 跨文本块也保持一致颜色。
编辑器顶部还提供常驻查找/替换栏：`Ctrl+F` / `Ctrl+H` 聚焦输入，`F3` / `Shift+F3` 循环导航，
可切换大小写匹配并执行当前或全部替换。右侧实时显示 UTF-16 文本对应的用户行列；全部替换在编辑器内只形成
一条撤销记录，因此一次 `Ctrl+Z` 即可恢复整批修改。多行选区按 `Tab` / `Shift+Tab` 会整体增加/减少一级
缩进，而不会用一个 Tab 覆盖源码；选区会随编辑和 Undo/Redo 保持，整批缩进同样只形成一条编辑器历史。
源码区右键菜单提供动态可用的撤销/重做、剪切/复制/粘贴/删除、全选、缩进和格式化，右键已选文本不会破坏
选区；菜单键或 `Shift+F10` 从光标位置打开同一菜单，`Ctrl+Shift+F` 格式化文档。源码区还提供上下文 XAML
补全：元素、属性、枚举/布尔值
和事件候选直接来自设计器现有的内置及自定义控件元数据，输入时自动筛选，也可用 `Ctrl+Space` 主动触发；
方向键导航、`Tab` / `Enter` 提交。属性提交会生成 `Name=""` 并把光标置于引号内。`Enter` 会保留当前缩进，
在成对标签之间自动建立内层空行；光标进入完整标签时会同时高亮匹配的开始/结束标签名。事件属性值除 `Auto`
外还会列出当前文档及关联用户 `.h/.cpp` 中参数签名兼容的函数，已被不同签名占用的名称不会混入候选。
底部“定位选区”按 stable ID 把画布选区定位到对应 XAML 元素，“选中控件”则从光标所在元素同步设计选区；
因此在源码中重命名控件后仍能保持选择。格式化也会按同一身份恢复当前元素，不再把光标重置到文档顶部；
整次规范重写只形成一个 Undo 单元，Undo 恢复格式化前的源码与选区，Redo 恢复规范源码与映射后的元素选区。
“应用”或 `Ctrl+S` 可在不关闭编辑器的情况下把当前有效 XAML 建立为独立 Designer Undo 检查点；应用后会立即
开启下一段实时预览，无变化应用不增加历史。此后取消只回滚最后检查点之后的草稿，已经应用的检查点及完整
选择会保留，并在返回主窗口时立即恢复可用的撤销入口。
...
工具栏“XAML”打开当前完整文档的源码编辑器。输入以 300ms 防抖解析并实时预览到主画布；语法或模型验证失败
只显示诊断并保留最后一次有效预览。此时“还原预览”可一键放弃无效或尚未应用的草稿并恢复该检查点；恢复
本身只形成一个文本 Undo 单元，撤销/重做会同时恢复操作前后的光标或选区。“格式化”使用规范 writer 原子重写当前有效文档，
同样只形成一个文本 Undo 单元并保存格式化前后的选区；`Ctrl+Enter` 接受，`Escape`
或关闭窗口取消。“应用”按钮和 `Ctrl+S` 会提交当前严格文档事务并立即从该端点开始下一段事务，因此每次有
变化的应用都是独立 Undo 检查点，无变化应用不占历史；最终取消只恢复到最近检查点，若从未应用则恢复打开
编辑器前的文档和完整选择。逐次键入仍不会污染 Designer 历史。内嵌查找/替换与错误定位共用 RichTextBox 的统一选区通知和滚动路径，
所以键盘、鼠标、程序化定位及撤销后的行列反馈保持一致。补全目录同样复用控件属性/事件元数据与自定义控件
manifest，不维护第二份易过期的 XAML schema；词法上下文、标签配对和缩进模型均可脱离窗口回归测试。
...
自定义控件通过 `RuntimeCustomControlRegistry` 注册，事件通过
`RuntimeEventHandlerRegistry` 路由到成员函数。这使它成为静态 C++ 构造与动态 XAML
...
前者只验证解析、事件契约和自定义控件工厂，后者还会实际材质化完整窗体并初始化运行时数据；
两者成功时都返回 0。
