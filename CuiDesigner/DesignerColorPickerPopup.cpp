...
namespace
{
	...
	static cui::core::Size GetTopLevelContentSize(Window* window)
	{
		if (!window) return { 1.0f, 1.0f };
		const auto viewport = window->GetContentViewportSizeDip();
		return {
			(std::max)(1.0f, std::floor(viewport.width)),
			(std::max)(1.0f, std::floor(viewport.height)) };
	}

	static cui::core::Size FitPopupSizeToWindow(
		Window* window, cui::core::Size preferred)
	{
		auto content = GetTopLevelContentSize(window);
		return {
			(std::min)(preferred.width, (std::max)(1.0f, content.width - 4.0f)),
			(std::min)(preferred.height, (std::max)(1.0f, content.height - 4.0f)) };
	}
}
...
{
	this->Focusable = true;
	_preferredSize = cui::core::Size{
		static_cast<float>(width), static_cast<float>(height) };
	Canvas::SetLeft(*this, 0.0f);
	Canvas::SetTop(*this, 0.0f);
	Width = static_cast<float>(width);
	Height = static_cast<float>(height);
	this->Background = D2D1_COLOR_F{ 0,0,0,0 };
	this->BorderBrush = D2D1_COLOR_F{ 0,0,0,0 };
...
void ColorPickerPopup::UpdateColorFromHsv()
{
	...
	cui::framework::EventAccess::Raise(
		...
		this->SelectedColor, CurrentValueText());
...
{
	Window* form = this->GetPresentationWindow();
	...
	if (form)
		(void)cui::framework::WindowAccess::CloseTransientPresentation(
			*form, this);
	if (form && form->GetKeyboardFocusedElement() == this)
		form->SetKeyboardFocus(owner && owner->IsVisible ? owner : nullptr, false);
	this->_owner = nullptr;
}
...
{
	if (!this->IsVisible && !_collapseCleanupPending) return false;
	outRect = GetAbsoluteBoundsDip();
	if (_hasAnchorRect)
	{
		const auto loc = this->GetAbsoluteLocationDip();
		D2D1_RECT_F anchorAbs = D2D1::RectF(
			loc.x + _anchorRect.left,
			loc.y + _anchorRect.top,
			loc.x + _anchorRect.right,
			loc.y + _anchorRect.bottom);
...
void ColorPickerPopup::ShowAt(Window* form, int x, int y, D2D1_COLOR_F initialColor)
{
	if (!form) return;
	this->_owner = nullptr;
	Control::PropagatePresentationWindow(this, form);
	this->Visibility = Visibility::Visible;
	_visiblePopup = true;
	_hasAnchorRect = false;
	SetFromColor(initialColor);
	const auto client = GetTopLevelContentSize(form);
	int clientW = static_cast<int>(client.width);
	int clientH = static_cast<int>(client.height);
	const auto popupSize = FitPopupSizeToWindow(form, _preferredSize);
	int minX = clientW > popupSize.width + 4.0f ? 2 : 0;
	int minY = clientH > popupSize.height + 4.0f ? 2 : 0;
	int maxX = (std::max)(minX,
		clientW - static_cast<int>(popupSize.width) - minX);
	int maxY = (std::max)(minY,
		clientH - static_cast<int>(popupSize.height) - minY);
	...
	if (!cui::framework::WindowAccess::OpenTransientPresentation(
		*form, this, TransientPresentationOptions{},
		[](Control& root)
		{
			static_cast<ColorPickerPopup&>(root).Hide(false);
		}))
	{
		Hide(false);
		return;
	}
	form->SetKeyboardFocus(this, false);
	SetExpanded(true);
}

void ColorPickerPopup::ShowAt(Control* relativeTo, const D2D1_RECT_F& anchorRect, D2D1_COLOR_F initialColor)
{
	if (!relativeTo || !relativeTo->GetPresentationWindow()) return;
	auto popupSize = FitPopupSizeToWindow(relativeTo->GetPresentationWindow(), _preferredSize);
	const auto abs = relativeTo->GetAbsoluteLocationDip();
	int x = (int)std::round(abs.x + anchorRect.left);
	int yBelow = (int)std::round(abs.y + anchorRect.bottom + 4.0f);
	int yAbove = (int)std::round(
		abs.y + anchorRect.top - popupSize.height - 4.0f);
	const auto client = GetTopLevelContentSize(relativeTo->GetPresentationWindow());
	int y = (yBelow + popupSize.height <= client.height - 2.0f || yAbove < 2)
		? yBelow : yAbove;
	ShowAt(relativeTo->GetPresentationWindow(), x, y, initialColor);
	this->_owner = relativeTo;
	const auto loc = this->GetActualLocationDip();
	_anchorRect = D2D1::RectF(
		abs.x + anchorRect.left - loc.x,
		abs.y + anchorRect.top - loc.y,
		abs.x + anchorRect.right - loc.x,
		abs.y + anchorRect.bottom - loc.y);
	_hasAnchorRect = true;
}

void ColorPickerPopup::Hide(bool confirm)
{
	if (!_visiblePopup && !this->IsVisible && !_animating) return;
	if (!_expanded && _animating && _animTargetProgress <= 0.001f) return;
	if (confirm)
		Confirm();
	else
		cui::framework::EventAccess::Raise(this->OnCancelled, this);
	SetExpanded(false);
}
...
{
	cui::framework::EventAccess::Raise(this->OnCleared, this);
	SetExpanded(false);
}
...
void ColorPickerPopup::OnRender()
{
...
bool ColorPickerPopup::ProcessInput(const InputReport& input)
{
	if (!this->IsEnabled || !this->IsVisible) return true;
	const bool inPanel = Control::ContainsPoint(input.X, input.Y);
	const bool draggingPicker = _dragSV || _dragHue || _dragAlpha;
	if (!draggingPicker && !inPanel && _hasAnchorRect
		&& PtInRectF(_anchorRect, (float)input.X, (float)input.Y))
	{
		if (_expanded && input.Kind == InputReportKind::PointerDown
	...
	{
		if (_dragSV) SetSVFromPoint(input.X, input.Y);
		else if (_dragHue) SetHueFromPoint(input.X, input.Y);
		else if (_dragAlpha) SetAlphaFromPoint(input.X, input.Y);
	...
	case InputReportKind::PointerDown:
	{
		...
		auto layout = CalcLayout();
		UpdateHover(input.X, input.Y);
		if (PtInRectF(layout.SvRect, (float)input.X, (float)input.Y))
		{
			_dragSV = true;
			SetSVFromPoint(input.X, input.Y);
			(void)CaptureMouse();
		}
		else if (PtInRectF(layout.HueRect, (float)input.X, (float)input.Y))
		{
			_dragHue = true;
			SetHueFromPoint(input.X, input.Y);
			(void)CaptureMouse();
		}
		else if (PtInRectF(layout.AlphaRect, (float)input.X, (float)input.Y))
		{
			_dragAlpha = true;
			SetAlphaFromPoint(input.X, input.Y);
			(void)CaptureMouse();
		}
	...
	{
		if (input.Key == Key::Escape) Hide(false);
		else if (input.Key == Key::Return) Hide(true);
