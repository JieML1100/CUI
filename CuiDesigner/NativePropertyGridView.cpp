...
#include "DesignerColorPickerPopup.h"
#include "DesignerDropDownPopup.h"
...
namespace
{
	...
	DependencyPropertyOptions<PropertyGridView, TValue> PropertyGridPropertyOptions(
		TValue defaultValue,
	...
	{
		DependencyPropertyOptions<PropertyGridView, TValue> options;
	...
	DependencyPropertyOptions<PropertyGridView, float> PropertyGridMetricOptions(
		float defaultValue, int order)
	{
		auto options = PropertyGridPropertyOptions(
			...
			DependencyPropertyFlags::AffectsArrange | DependencyPropertyFlags::AffectsRender);
		options.Coerce = [](
	...
	DependencyPropertyOptions<PropertyGridView, D2D1_COLOR_F> PropertyGridColorOptions(
...
void PropertyGridView::RegisterDependencyProperties()
{
	Control::RegisterDependencyProperties();
	static const bool registered = []
	{
#define CUI_REGISTER_PROPERTY_GRID_BOOL(name, propertyName, defaultValue, order) \
		DependencyPropertyRegistry::Register<PropertyGridView, bool>(propertyName, \
		...
		CUI_REGISTER_PROPERTY_GRID_BOOL(AlternatingRows, L"AlternatingRows", true, 30);
		{
			...
			DependencyPropertyRegistry::Register<PropertyGridView, bool>(L"AllowEditing",
...
#define CUI_REGISTER_PROPERTY_GRID_METRIC(name, propertyName, defaultValue, order) \
		{ \
			...
			DependencyPropertyRegistry::Register<PropertyGridView, float>(propertyName, \
				[](PropertyGridView& target) { return target.name; }, \
...
#undef CUI_REGISTER_PROPERTY_GRID_METRIC
		...
		DependencyPropertyRegistry::Register<PropertyGridView, int>(L"MouseWheelStep",
		...
		hoverOptions.Design.Persistence = DependencyPropertyPersistence::Transient;
		DependencyPropertyRegistry::Register<PropertyGridView, int>(L"HoveredIndex",
		...
		scrollOptions.Design.Persistence = DependencyPropertyPersistence::Transient;
		DependencyPropertyRegistry::Register<PropertyGridView, float>(L"ScrollYOffset",
...
#define CUI_REGISTER_PROPERTY_GRID_COLOR(name, propertyName, defaultValue, order) \
		DependencyPropertyRegistry::Register<PropertyGridView, D2D1_COLOR_F>(propertyName, \
			[](PropertyGridView& target) { return target.name; }, \
...
PropertyGridView::PropertyGridView(int x, int y, int width, int height)
{
	...
	this->Background = cui::theme::palette::Surface;
	this->BorderBrush = cui::theme::palette::Border;
	this->Foreground = cui::theme::palette::TextPrimary;
}
...
{
	if (index < 0 || index >= (int)this->Items.size()) return false;
	auto& item = this->Items.MutableAt(static_cast<size_t>(index));
...
PropertyGridView::Layout PropertyGridView::CalcLayout(const std::vector<RowInfo>& rows) const
{
	...
	const float width = actualSize.width;
	const float height = actualSize.height;
...
bool PropertyGridView::HandlesNavigationKey(Key key) const
{
	switch (key)
	{
	case Key::Up:
...
void PropertyGridView::DrawItemRow(D2DGraphics* d2d, const RowInfo& row, const D2D1_RECT_F& rect, int visibleItemOrdinal)
{
	...
	class Font* fontObj = this->GetRenderFont();
	D2D1_COLOR_F nameColor = item.ReadOnly
		|| item.ValueType == PropertyGridValueType::ReadOnly
		? this->ReadOnlyForeColor : this->Foreground.Color;
	...
	D2D1_COLOR_F valueColor = item.ReadOnly
		|| item.ValueType == PropertyGridValueType::ReadOnly
		? this->ReadOnlyForeColor : this->Foreground.Color;
	...
	if (_editing && _editingIndex == row.ItemIndex)
	{
		...
		const bool focused = this->GetPresentationWindow() && this->GetPresentationWindow()->GetKeyboardFocusedElement() == this;
...
bool PropertyGridView::ActivateItem(int index)
{
	if (index < 0 || index >= (int)this->Items.size()) return false;
	SelectItem(index);
	cui::framework::EventAccess::Raise(this->OnItemClick, this, index);
	return true;
}
...
bool PropertyGridView::RequestReset(int index)
{
	...
	SelectItem(index);
	cui::framework::EventAccess::Raise(this->OnResetRequested, this, index);
	return true;
}
...
void PropertyGridView::MoveEditCaret(int delta, bool extendSelection)
{
	if (!_editing) return;
	EditEnsureSelectionInRange();
	if (delta == 0) return;
...
bool PropertyGridView::TryGetTextInputCaretRect(D2D1_RECT_F& outRect)
{
	if (!this->GetPresentationWindow() || !_editing) return false;
	auto rows = BuildRows();
	auto layout = CalcLayout(rows);
	D2D1_RECT_F valueRect{};
	if (!GetValueRectForItem(_editingIndex, rows, layout, valueRect)) return false;
	EditEnsureSelectionInRange();
...
bool PropertyGridView::ApplyTextInput(
	const TextCompositionEventArgs& input)
{
	if (input.Text.empty()) return false;
	if (!_editing && SelectedIndex >= 0
		&& SelectedIndex < static_cast<int>(Items.size()))
	{
		const auto type = Items[SelectedIndex].ValueType;
		if (IsEditableItem(SelectedIndex)
			&& (type == PropertyGridValueType::Text
				|| type == PropertyGridValueType::Number
				|| type == PropertyGridValueType::EditableEnum))
			(void)BeginEdit(SelectedIndex);
	}
	if (!_editing) return false;
	const std::wstring previous = _editingText;
	InputEditText(input.Text);
	return _editingText != previous;
}
...
void PropertyGridView::OpenColorPickerEditor(int index, const D2D1_RECT_F& valueRect)
{
	...
	if (this->_colorPicker &&
		cui::framework::WindowAccess::IsTransientPresentationOpen(
			*this->GetPresentationWindow(), this->_colorPicker) &&
		this->_colorPicker->IsVisible &&
	...
	Control::PropagatePresentationWindow(
		this->_colorPicker, this->GetPresentationWindow());
	(void)cui::framework::DependencyPropertyAccess::SetValue(
		*this->_colorPicker, L"FontFamily", BindingValue(this->FontFamily),
		DependencyPropertyValueSource::Template);
	(void)cui::framework::DependencyPropertyAccess::SetValue(
		*this->_colorPicker, L"FontSize", BindingValue(this->FontSize),
		DependencyPropertyValueSource::Template);
	this->_colorPicker->AccentColor = this->AccentColor;

	cui::framework::EventAccess::Clear(this->_colorPicker->OnColorChanged);
	cui::framework::EventAccess::Clear(this->_colorPicker->OnColorConfirmed);
	cui::framework::EventAccess::Clear(this->_colorPicker->OnCleared);
	cui::framework::EventAccess::Clear(this->_colorPicker->OnCancelled);
...
void PropertyGridView::ToggleDropDownEditor(int index, const D2D1_RECT_F& valueRect)
{
	...
	(void)cui::framework::DependencyPropertyAccess::SetValue(
		*this->_dropDownPopup, L"FontFamily", BindingValue(this->FontFamily),
		DependencyPropertyValueSource::Template);
	(void)cui::framework::DependencyPropertyAccess::SetValue(
		*this->_dropDownPopup, L"FontSize", BindingValue(this->FontSize),
		DependencyPropertyValueSource::Template);
	this->_dropDownPopup->DropBackColor = this->EditBackColor;
	this->_dropDownPopup->Foreground = this->EditForeColor;
	...
	cui::framework::EventAccess::Clear(
		this->_dropDownPopup->SelectionChanged);
	...
	cui::framework::EventAccess::Clear(this->_dropDownPopup->Closed);
	...
	this->_dropDownPopupIndex = index;
	SetCurrentSelectedIndex(index);
	this->_dropDownPopup->ShowAt(this->GetPresentationWindow(), this,
		D2D1::RectF(x, y, x + w, y + h),
		item.Options, selected, w, h, 4);
	this->InvalidateVisual();
}

void PropertyGridView::OnRender()
{
	...
	this->BeginRender();
	{
		d2d->FillRoundRect(Border * 0.5f, Border * 0.5f,
			std::max(0.0f, width - Border),
			std::max(0.0f, height - Border),
			this->Background.Color, this->CornerRadius);
		...
		if (Border > 0.0f)
			d2d->DrawRoundRect(Border * 0.5f, Border * 0.5f,
				std::max(0.0f, width - Border),
				std::max(0.0f, height - Border),
				this->BorderBrush.Color, Border, this->CornerRadius);
		if (!this->IsEnabled)
...
bool PropertyGridView::ProcessInput(const InputReport& input)
{
	...
	case InputReportKind::PointerMove:
	{
		...
		if (_dragSlider && _sliderDragIndex >= 0)
		{
			...
			if (GetValueRectForItem(
				...
				UpdateSliderFromPoint(
					_sliderDragIndex, static_cast<float>(input.X), valueRect);
		}
		else if (_dragEditSelection && _editing)
		{
			...
			if (GetValueRectForItem(_editingIndex, rows, layout, valueRect))
				UpdateEditingSelectionFromMousePoint(input.X, input.Y, valueRect);
		}
		else if (_dragVScroll)
			UpdateScrollByThumb((float)input.Y);
		else if (_dragSplitter)
		{
			...
			auto rows = BuildRows();
			auto layout = CalcLayout(rows);
			SetCurrentNameColumnWidth(std::clamp((float)input.X, 48.0f,
	...
	case InputReportKind::PointerDown:
	{
		...
		auto rows = BuildRows();
		auto layout = CalcLayout(rows);
		if (layout.NeedVScroll && PtInRectF(layout.ScrollThumbRect,
			(float)input.X, (float)input.Y))
		{
			_dragVScroll = true;
			_scrollThumbGrabOffsetY = (float)input.Y - layout.ScrollThumbRect.top;
			return true;
		}
		if (layout.NeedVScroll && PtInRectF(layout.ScrollTrackRect,
			(float)input.X, (float)input.Y))
		{
			SetScrollOffset(this->ScrollYOffset
				+ ((float)input.Y < layout.ScrollThumbRect.top
					? -RectHeight(layout.ContentRect)
					: RectHeight(layout.ContentRect)));
			return true;
		}
		if (IsOverSplitter(input.X, input.Y))
		{
			_dragSplitter = true;
			return true;
		}
		...
		{
			auto rect = GetRowRect(row, layout);
			if (!PtInRectF(rect, (float)input.X, (float)input.Y)) continue;
			handledRow = true;
			if (row.IsCategory)
			{
				CancelEdit();
			...
			{
				int valueIndex = -1;
				bool inValue = IsValueCell(input.X, input.Y, rows, layout, valueIndex);
				...
				if (canReset && PtInRectF(
					GetResetRect(rect), (float)input.X, (float)input.Y))
				{
					CancelEdit();
					CloseDropDownEditor();
					CloseColorPickerEditor();
					...
					&& inValue && IsEditableItem(row.ItemIndex))
				{
					...
					UpdateSliderFromPoint(
						row.ItemIndex, static_cast<float>(input.X), GetValueRect(rect));
					(void)CaptureMouse();
					break;
				}
				if (inValue && IsEditableItem(row.ItemIndex))
				{
					...
					if (type == PropertyGridValueType::Bool)
					{
						...
						input.X >= GetValueRect(rect).right - 24.0f)
						ToggleDropDownEditor(row.ItemIndex, GetValueRect(rect));
					...
					else
					{
						...
						if (_editing && _editingIndex == row.ItemIndex)
						{
							SetEditingCaretFromMousePoint(input.X, input.Y, GetValueRect(rect));
	...
	case InputReportKind::PointerUp:
	{
		...
		if (_dragSlider)
		{
			...
			if (GetValueRectForItem(sliderIndex, rows, layout, valueRect))
				UpdateSliderFromPoint(
				sliderIndex, static_cast<float>(input.X), valueRect);
			_dragSlider = false;
			_sliderDragIndex = -1;
	...
	case InputReportKind::PointerDoubleClick:
	{
		...
		auto rows = BuildRows();
		auto layout = CalcLayout(rows);
		int index = HitTestItem(input.X, input.Y);
		if (index >= 0)
		{
			D2D1_RECT_F valueRect{};
	...
	case InputReportKind::KeyDown:
	{
		if (_editing)
		{
			...
			if (input.HasModifier(ModifierKeys::Control))
			{
				...
				{
					_editSelectionStart = 0;
					_editSelectionEnd = static_cast<int>(_editingText.size());
					InvalidateVisual();
					return true;
				}
				if (input.Key == Key::C
					|| input.Key == Key::X)
				{
					WriteClipboardText(GetPresentationWindow() ? GetPresentationWindow()->Handle : nullptr,
						EditGetSelectedString());
					if (input.Key == Key::X) BackspaceEdit();
					return true;
				}
				if (input.Key == Key::V)
				{
					std::wstring clipboardText;
					if (TryReadClipboardText(
						GetPresentationWindow() ? GetPresentationWindow()->Handle : nullptr,
						clipboardText)) InputEditText(clipboardText);
					return true;
				}
			}
			switch (input.Key)
			{
			case Key::Return: CommitEdit(); break;
			case Key::Escape: CancelEdit(); break;
			case Key::Back: BackspaceEdit(); break;
			case Key::Delete: DeleteEdit(); break;
			case Key::Left: MoveEditCaret(-1, input.HasModifier(ModifierKeys::Shift)); break;
			case Key::Right: MoveEditCaret(1, input.HasModifier(ModifierKeys::Shift)); break;
			case Key::Home:
				_editSelectionEnd = 0;
			...
			auto e = input.CreateKeyEventArgs();
		...
		switch (input.Key)
		{
		case Key::Up: selectVisible(pos <= 0 ? 0 : pos - 1); break;
		case Key::Down: selectVisible(pos < 0 ? 0 : pos + 1); break;
		case Key::Home: selectVisible(0); break;
		case Key::End: selectVisible((int)visibleItems.size() - 1); break;
		case Key::PageUp: selectVisible(std::max(0, pos - 8)); break;
		case Key::PageDown: selectVisible(pos < 0 ? 0 : pos + 8); break;
		case Key::Return:
		case Key::F2:
	...
	case InputReportKind::CaptureLost:
	{
		if (_dragSlider)
		{
			...
			if (index >= 0)
				cui::framework::EventAccess::Raise(
					this->OnEditCanceled, this, index);
		}
		_dragVScroll = false;
		_dragSplitter = false;
		_dragEditSelection = false;
