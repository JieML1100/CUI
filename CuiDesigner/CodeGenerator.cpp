		...
		if (!lv->Items.empty())
		{
			...
			for (int i = 0; i < (int)lv->Items.size(); i++)
			{
				...
				for (const auto& sub : item.SubItems)
					code << indentStr << itemVar << ".SubItems.push_back(L\"" << EscapeWStringLiteral(sub) << "\");\n";
				if (item.Checked)
				...
				if (item.Selected)
					code << indentStr << name << "->SelectedIndex = " << i << ";\n";
			}
		}
	}
		...
		if (!pg->Items.empty())
		{
			...
			for (int i = 0; i < (int)pg->Items.size(); i++)
			{
				...
				if (!item.Description.empty())
					code << indentStr << itemVar << ".Description = L\"" << EscapeWStringLiteral(item.Description) << "\";\n";
				if (item.ReadOnly)
...
std::string CodeGenerator::GenerateCpp()
{
	...
	emitControl = [&](const std::shared_ptr<DesignerControl>& dc, const std::string& parentExpr, int indent)
	{
		...
		{
			cpp << indentStr << parentExpr << "->AddToolItem(" << childVar << ");\n";
		}
