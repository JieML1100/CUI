...
- `TreeView` - 树形视图
- `GridView` - 数据网格
- `PagedGridView` - GridView 分页表格
- `ListView` - 列表/明细/磁贴/图标视图
- `ListBox` - 简单列表选择框
- `PropertyGrid` - 运行时属性表

**导航与日期:**
- `NavigationView` / `SideBar` - 侧栏导航
- `BreadcrumbBar` - 面包屑路径导航
...
```cpp
auto expander = AddControl(new Expander(L"高级设置", 20, 450, 360, 150));
expander->Padding = Thickness(12);
expander->AddControl(new Label(L"折叠内容", 0, 0));
...
list->AddColumn(ListViewColumn(L"名称", 160));
list->AddColumn(ListViewColumn(L"状态", 100));

ListViewItem item(L"任务 1", L"Ready");
item.Checked = true;
item.SubItems.push_back(item.SubText);
list->AddItem(item);

auto listBox = AddControl(new ListBox(360, 20, 180, 220));
listBox->AddItem(ListViewItem(L"全部"));
...
props->AddProperty(L"外观", L"标题", L"设置", PropertyGridValueType::Text);
props->AddProperty(L"外观", L"启用", L"True", PropertyGridValueType::Bool);

PropertyGridItem mode(L"行为", L"模式", L"普通", PropertyGridValueType::Enum);
mode.Options = { L"紧凑", L"普通", L"宽松" };
props->AddItem(mode);

props->OnValueChanged += [](PropertyGridView* sender, int index,
    std::wstring oldValue, std::wstring newValue) {
    // index 对应 sender->Items[index]
};
```

#### NavigationView / SideBar / BreadcrumbBar 导航

```cpp
auto nav = AddControl(new NavigationView(20, 20, 220, 360));
nav->HeaderText = L"工作台";
nav->AddHeader(L"主导航");
nav->AddItem(L"概览", L"overview");
nav->Items.back().BadgeText = L"3";
nav->AddItem(L"资源", L"assets");
nav->AddSeparator();
nav->AddItem(L"设置", L"settings");
nav->SelectItem(1);

nav->OnItemClick += [](NavigationView* sender, int index) {
    // sender->Items[index].Value 可作为业务路由键
};

auto breadcrumb = AddControl(new BreadcrumbBar(260, 20, 360, 32));
breadcrumb->SetPath({ L"应用", L"资源", L"详情" });
breadcrumb->OnItemClick += [](BreadcrumbBar* sender, int index) {
    // 点击任意路径段可回退到对应层级
};
```

#### CalendarView / DateRangePicker 日期范围
...
calendar->OnSelectionChanged += [](CalendarView* sender) {
    ...
    if (range.HasStart && range.HasEnd) {
        // range.Start / range.End 为选中的闭区间
    }
};

auto rangePicker = AddControl(new DateRangePicker(L"选择日期范围", 360, 20, 240, 30));
rangePicker->OnRangeChanged += [](DateRangePicker* sender) {
    auto range = sender->GetRange();
    // sender->Text 会自动显示 yyyy-mm-dd - yyyy-mm-dd
};
```
...
paged->PageSize = 50;
paged->AddColumn(GridViewColumn(L"名称", 150, ColumnType::Text));
paged->AddColumn(GridViewColumn(L"状态", 120, ColumnType::ComboBox));

for (int i = 0; i < 500; i++) {
    GridViewRow row;
    row.Cells = { L"项目 " + std::to_wstring(i + 1), L"Ready" };
    paged->AddRow(row);
}
paged->RefreshPage();
paged->OnPageChanged += [](PagedGridView* sender, int oldPage, int newPage) {
    // sender->Grid 是完整 GridView，可继续使用按钮列、Check 列、ComboBox 列等能力
};
```
...
toastHost->ShowToast(L"保存完成", L"配置已经写入磁盘。", ToastKind::Success);

toastHost->OnToastClick += [](ToastHost* sender, int index) {
    // index 对应 sender->Toasts[index]
};
```
