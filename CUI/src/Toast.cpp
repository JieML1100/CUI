...
bool ToastHost::RemoveExpired()
{
	...
	for (int i = (int)this->Toasts.size() - 1; i >= 0; i--)
	{
		...
		if (toast.Dismissing)
		{
			if (now - toast.DismissStartTick
				>= EffectiveAnimationDuration(toast.DismissDurationMs))
			{
...
void ToastHost::Update()
{
	...
	this->BeginRender();
	{
		for (int i = 0; i < (int)indices.size(); i++)
		{
			...
			if (toast.Dismissing)
			{
				const UINT dismissDuration =
					EffectiveAnimationDuration(toast.DismissDurationMs);
				dismissProgress = dismissDuration > 0
					? std::clamp((float)(now - toast.DismissStartTick) / (float)dismissDuration, 0.0f, 1.0f)
					: 1.0f;
