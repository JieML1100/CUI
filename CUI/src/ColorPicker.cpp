...
void ColorPicker::EnsureBindingPropertiesRegistered()
{
	Control::EnsureBindingPropertiesRegistered();
	static const bool registered = []
	{
		using Handler = BindingPropertyMetadata::ChangeHandler;
		BindingPropertyRegistry::Register<ColorPicker, D2D1_COLOR_F>(L"SelectedColor",
			[](ColorPicker& target) { return target.SelectedColor; },
			[](ColorPicker& target, const D2D1_COLOR_F& value) { target.SelectedColor = value; },
			[](ColorPicker& target, Handler handler, DataSourceUpdateMode)
			{
				return target.OnColorChanged.Subscribe(
					[handler = std::move(handler)](ColorPicker*, D2D1_COLOR_F, D2D1_COLOR_F, std::wstring)
					{
						handler();
					});
			});
...
float ColorPicker::CurrentHoverProgress()
{
	...
	const UINT duration = EffectiveAnimationDuration(_hoverAnimDurationMs);
