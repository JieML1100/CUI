...
void DateTimePicker::EnsureBindingPropertiesRegistered()
{
	...
	static const bool registered = []
	{
		...
		BindingPropertyRegistry::Register<DateTimePicker, SYSTEMTIME>(L"Value",
			...
			[](DateTimePicker& target, Handler handler, DataSourceUpdateMode)
			{
...
static D2D1_POINT_2F DatePickerRotatePoint(const D2D1_POINT_2F& point, float cx, float cy, float angle)
{
...
static void DrawDatePickerChevron(D2DGraphics* d2d, float cx, float cy, float size, float progress, D2D1_COLOR_F color)
{
	if (!d2d) return;
	...
	p1 = DatePickerRotatePoint(p1, cx, cy, angle);
	p2 = DatePickerRotatePoint(p2, cx, cy, angle);
	p3 = DatePickerRotatePoint(p3, cx, cy, angle);
	d2d->DrawLine(p1, p2, color, 1.8f);
	d2d->DrawLine(p2, p3, color, 1.8f);
}

static void DrawDatePickerArrow(D2DGraphics* d2d, const D2D1_RECT_F& rect, bool next, D2D1_COLOR_F color)
{
	const float cx = rect.left + (rect.right - rect.left) * 0.5f;
	const float cy = rect.top + (rect.bottom - rect.top) * 0.5f;
	const float halfW = 3.2f;
	const float halfH = 5.3f;
	if (next)
	{
...
float DateTimePicker::CurrentDropProgress()
{
	if (IsInlineTimeMode())
	{
		_dropProgress = 0.0f;
...
bool DateTimePicker::IsDropDownVisible()
{
	if (IsInlineTimeMode()) return false;
	return this->Expand || _animating || _dropProgress > 0.001f;
}

bool DateTimePicker::IsAnimationRunning()
{
	CurrentDropProgress();
	return _animating || _collapseCleanupPending;
}

bool DateTimePicker::GetAnimatedInvalidRect(D2D1_RECT_F& outRect)
{
...
CursorKind DateTimePicker::QueryCursor(int localX, int localY)
{
	...
	if (localY >= 0 && localY <= this->Height)
	{
		if (IsInlineTimeMode())
		{
			...
			{
				auto part = HitTestPart(layout, localX, localY, day);
...
void DateTimePicker::Update()
{
	...
	this->BeginRender(size.width, size.height);
	{
		...
		if (isUnderMouse && !IsDropDownVisible())
			baseColor = blendColor(this->BackColor, this->HeaderHoverBackColor, 0.85f);

		d2d->FillRoundRect(0, 0, (float)this->Width, (float)this->Height, baseColor, round);
		if ((isUnderMouse || IsDropDownVisible()) && !inlineTime)
			d2d->FillRoundRect(0, 0, (float)this->Width, (float)this->Height, this->HeaderHoverBackColor, round);
		...
		auto drawTimeField = [&](const D2D1_RECT_F& fieldRect, const D2D1_RECT_F& upRect, const D2D1_RECT_F& downRect,
			EditField field, const std::wstring& text, bool hoverUp, bool hoverDown)
			{
				...
				d2d->FillRoundRect(fieldRect.left, fieldRect.top, fieldRect.right - fieldRect.left, fieldRect.bottom - fieldRect.top, this->PanelBackColor, 6.0f);
				...
				d2d->FillRoundRect(spinLeft, fieldRect.top + 3.0f, fieldRect.right - spinLeft - 3.0f, fieldRect.bottom - fieldRect.top - 6.0f, spinBackColor, 4.0f);
				if (hoverUp)
					d2d->FillRoundRect(upRect.left + 1.0f, upRect.top + 1.0f, upRect.right - upRect.left - 2.0f, upRect.bottom - upRect.top - 2.0f, this->HoverColor, 3.0f);
				if (hoverDown)
					d2d->FillRoundRect(downRect.left + 1.0f, downRect.top + 1.0f, downRect.right - downRect.left - 2.0f, downRect.bottom - downRect.top - 2.0f, this->HoverColor, 3.0f);
				...
				d2d->DrawRoundRect(fieldRect.left, fieldRect.top, fieldRect.right - fieldRect.left, fieldRect.bottom - fieldRect.top, borderColor, borderWidth, 6.0f);
				...
				auto drawArrowButton = [&](const D2D1_RECT_F& buttonRect, bool up, bool hovered)
					{
						...
						float halfW = std::clamp(std::min(buttonWidth, buttonHeight) * 0.18f, 2.5f, 4.0f);
						float halfH = halfW * 0.78f;
						auto col = hovered ? this->ForeColor : this->SecondaryTextColor;
						if (up)
						{
							d2d->DrawLine(D2D1::Point2F(cx - halfW, cy + halfH), D2D1::Point2F(cx, cy - halfH), col, 1.5f);
							d2d->DrawLine(D2D1::Point2F(cx, cy - halfH), D2D1::Point2F(cx + halfW, cy + halfH), col, 1.5f);
						}
						else
						{
							d2d->DrawLine(D2D1::Point2F(cx - halfW, cy - halfH), D2D1::Point2F(cx, cy + halfH), col, 1.5f);
		...
		else
		{
			...
			float textX = 10.0f;
			float textY = std::max(0.0f, ((float)this->Height - textSize.height) * 0.5f);
			d2d->DrawString(this->Text, textX, textY, std::max(1.0f, (float)this->Width - textX - 34.0f), textSize.height + 2.0f, this->ForeColor, font);

			{
				...
				const float cx = (float)this->Width - 16.0f;
				const float cy = h * 0.5f;
				DrawDatePickerChevron(d2d, cx, cy, iconSize, dropProgress, this->ForeColor);
			}
		}
		...
		if (IsDropDownVisible() && !inlineTime && dropProgress > 0.001f)
		{
			...
			if (GetLayoutMetrics(layout))
			{
				...
				auto drawToggle = [&](D2D1_RECT_F rect, bool enabled, const wchar_t* label, bool hovered)
					{
						D2D1_COLOR_F fill = enabled ? this->SelectedBackColor : blendColor(this->DropBackColor, this->DropBorderColor, 0.20f);
						if (hovered)
							fill = this->HoverColor;
						auto drawRect = rect;
						d2d->FillRoundRect(drawRect.left, drawRect.top, drawRect.right - drawRect.left, drawRect.bottom - drawRect.top, fill, 12.0f);
						d2d->DrawRoundRect(drawRect.left, drawRect.top, drawRect.right - drawRect.left, drawRect.bottom - drawRect.top,
							enabled ? this->AccentColor : this->DropBorderColor, enabled ? 1.2f : 1.0f, 12.0f);
						D2D1_COLOR_F textColor = enabled ? this->SelectedForeColor : this->ForeColor;
						auto ts = font->GetTextSize(label);
						float tx = drawRect.left + ((drawRect.right - drawRect.left) - ts.width) * 0.5f;
						float ty = drawRect.top + ((drawRect.bottom - drawRect.top) - ts.height) * 0.5f;
				...
				{
					// 月份标题与切换
					auto prevButtonRect = layout.prevRect;
					auto nextButtonRect = layout.nextRect;
					if (_hoverPart == HitPart::PrevMonth)
						d2d->FillRoundRect(prevButtonRect.left, prevButtonRect.top, prevButtonRect.right - prevButtonRect.left, prevButtonRect.bottom - prevButtonRect.top, this->HoverColor, 4.0f);
					if (_hoverPart == HitPart::NextMonth)
						d2d->FillRoundRect(nextButtonRect.left, nextButtonRect.top, nextButtonRect.right - nextButtonRect.left, nextButtonRect.bottom - nextButtonRect.top, this->HoverColor, 4.0f);

					DrawDatePickerArrow(d2d, prevButtonRect, false, this->ForeColor);
					DrawDatePickerArrow(d2d, nextButtonRect, true, this->ForeColor);
					...
					for (int i = 0; i < layout.gridRows * 7; i++)
					{
						...
						const float cellPad = std::max(2.0f, this->ItemPadding);
						if (isSelectedDate)
						{
							d2d->FillRoundRect(rect.left + cellPad, rect.top + cellPad, (rect.right - rect.left) - cellPad * 2.0f, (rect.bottom - rect.top) - cellPad * 2.0f, this->SelectedBackColor, 7.0f);
							const float accentW = 3.0f;
							d2d->FillRoundRect(rect.left + cellPad, rect.top + 7.0f, accentW, std::max(5.0f, (rect.bottom - rect.top) - 14.0f), this->AccentColor, accentW * 0.5f);
						}
						else if (isHover)
							d2d->FillRoundRect(rect.left + cellPad, rect.top + cellPad, (rect.right - rect.left) - cellPad * 2.0f, (rect.bottom - rect.top) - cellPad * 2.0f, this->HoverColor, 7.0f);
						else if (isToday)
							d2d->DrawRoundRect(rect.left + cellPad, rect.top + cellPad, (rect.right - rect.left) - cellPad * 2.0f, (rect.bottom - rect.top) - cellPad * 2.0f, this->AccentColor, 1.0f, 7.0f);
						...
						D2D1_COLOR_F dayColor = isSelectedDate ? this->SelectedForeColor : this->ForeColor;
		...
		if (!this->Enable)
		{
			d2d->FillRect(0.0f, 0.0f, size.width, size.height, blendColor(this->DropBackColor, Colors::White, 0.35f));
		}
	}
	this->EndRender();
	...
	case WM_MOUSEWHEEL:
	{
		if (this->Expand || IsInlineTimeMode())
		{
			...
			if (hasLayout)
			{
				...
				if (layout.showTime && localY >= (int)layout.timeTop)
				{
...
int DateTimePicker::DaysInMonth(int year, int month)
{
	...
	int normalizedMonth = std::clamp(month, 1, 12);
	int daysInSelectedMonth = days[normalizedMonth - 1];
...
bool DateTimePicker::GetLayoutMetrics(LayoutMetrics& out)
{
	...
	const float pad = 8.0f;
	const float baseTop = DropdownTop();
	out.contentLeft = pad;
	out.contentWidth = std::max(0.0f, (float)this->Width - pad * 2.0f);
...
bool DateTimePicker::HitTestDayCell(const LayoutMetrics& layout, int localX, int localY, int& outDay) const
{
	...
	int col = (int)((localX - gridLeft) / layout.cellWidth);
	int row = (int)((localY - gridTop) / layout.cellHeight);
	col = std::clamp(col, 0, 6);
	row = std::clamp(row, 0, 5);
	int dayCellIndex = row * 7 + col;
	int first = FirstWeekday(_viewYear, _viewMonth);
	int day = dayCellIndex - first + 1;
	int days = DaysInMonth(_viewYear, _viewMonth);
...
void DateTimePicker::UpdateHoverState(int localX, int localY)
{
	LayoutMetrics layout{};
...
DateTimePicker::HitPart DateTimePicker::HitTestPart(const LayoutMetrics& layout, int localX, int localY, int& outDay) const
{
	outDay = -1;
	auto inRect = [&](const D2D1_RECT_F& r) -> bool
		{
			return localX >= r.left && localX <= r.right && localY >= r.top && localY <= r.bottom;
		};
	...
	if (layout.showDate)
	{
		...
		if (HitTestDayCell(layout, localX, localY, outDay)) return HitPart::DayCell;
	}
	if (layout.showTime)
	{
