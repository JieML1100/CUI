...
D2D1_RECT_F TabControl::GetTitleRect(int index)
{
	...
	{
	case TabControlTitlePosition::Bottom:
		return D2D1_RECT_F{ titleWidth * indexStart + titleGap * 0.5f, (std::max)(0.0f, actualHeight - titleHeight) + titleGap * 0.5f,
			titleWidth * indexEnd - titleGap * 0.5f, actualHeight - titleGap * 0.5f };
	case TabControlTitlePosition::Left:
		return D2D1_RECT_F{ titleGap * 0.5f, titleHeight * indexStart + titleGap * 0.5f,
			(std::min)(actualWidth, titleWidth) - titleGap * 0.5f, titleHeight * indexEnd - titleGap * 0.5f };
	case TabControlTitlePosition::Right:
		return D2D1_RECT_F{ (std::max)(0.0f, actualWidth - titleWidth) + titleGap * 0.5f, titleHeight * indexStart + titleGap * 0.5f,
			actualWidth - titleGap * 0.5f, titleHeight * indexEnd - titleGap * 0.5f };
	case TabControlTitlePosition::Top:
	default:
		return D2D1_RECT_F{ titleWidth * indexStart + titleGap * 0.5f, titleGap * 0.5f,
			titleWidth * indexEnd - titleGap * 0.5f, (std::min)(actualHeight, titleHeight) - titleGap * 0.5f };
	}
}
...
bool TabControl::ProcessMessage(UINT message, WPARAM wParam, LPARAM lParam, int xof, int yof)
{
	...
	if (this->Count > 0)
	{
		...
		if (message == WM_MOUSEMOVE || message == WM_LBUTTONDOWN || message == WM_RBUTTONDOWN || message == WM_MBUTTONDOWN)
		{
			...
			if (this->_hoverTitleIndex != hoverIndex)
			{
				this->_hoverTitleIndex = hoverIndex;
				this->PostRender();
			}
		}
		else if (message == WM_MOUSELEAVE && this->_hoverTitleIndex != -1)
		{
		...
		if (message == WM_LBUTTONDOWN && TryGetTitleIndexAt(xof, yof, newSelected))
		{
