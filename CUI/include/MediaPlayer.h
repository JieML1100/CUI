...
public:
	...
	MediaPlayerCallback(MediaPlayer* player, UINT64 mediaLoadGeneration);
...
inline void MediaPlayerCallback::DetachPlayer()
{
...
class MediaPlayer : public Control
{
	...
	friend struct MediaPlayerRegressionTestAccess;
public:
	/**
	...
	/** @brief 播放状态发生变化。 */
	using MediaStateChangedEvent = Event<void(
		class MediaPlayer*, PlayState oldState, PlayState newState)>;
	/** @brief 携带 HRESULT 的详细媒体错误事件。 */
	using MediaErrorEvent = Event<void(class MediaPlayer*, HRESULT error)>;
...
private:
	...
	class PlaybackTransitionLease final
	{
		...
		explicit PlaybackTransitionLease(MediaPlayer* owner) noexcept
			: _owner(owner) {}
	...
	private:
		MediaPlayer* _owner = nullptr;
	};
	struct DeferredPlaybackNotifications final
	{
		bool StateChanged = false;
		PlayState OldState = PlayState::Stopped;
		PlayState NewState = PlayState::Stopped;
		...
		bool AutoPlay = false;
		UINT64 AutoPlayExplicitCommandGeneration = 0;
	};

	// ========== 播放状态 ==========
	std::atomic<PlayState> _playState{ PlayState::Stopped };
	...
	bool CommitPlayState(PlayState value, PlayState& oldValue) noexcept;
	void SetPlayState(PlayState value);
	void RaisePlayStateChanged(PlayState oldValue, PlayState value);
...
public:
	...
	/** @brief 播放状态实际变化时触发。 */
	MediaStateChangedEvent OnStateChanged;
...
public:
	...
	/** @brief 停止播放并释放当前媒体资源；控件可再次 Load。 */
	void Close();
	...
	bool IsPlaying() const { return _playState.load() == PlayState::Playing; }
	bool IsPaused() const { return _playState.load() == PlayState::Paused; }
	bool IsStopped() const { return _playState.load() == PlayState::Stopped; }
